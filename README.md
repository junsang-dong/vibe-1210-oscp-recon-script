...
**Vercel 배포:**
Vercel Dashboard → Settings → Environment Variables에서 `OPENAI_API_KEY`를 추가하세요.
...
1. **샘플 데이터 선택**: 6개의 샘플 시나리오 중 하나를 선택하여 로드합니다.
2. **테스트 정보 입력**: 테스트 날짜, 테스터 이름, 타겟 정보를 확인하거나 수정합니다.
3. **취약점 추가/수정**: "취약점 추가" 버튼으로 취약점을 추가하거나 기존 취약점을 수정합니다.
4. **보고서 생성**: "OSCP 보고서 자동 생성" 버튼을 클릭하여 GPT-4로 보고서를 생성합니다.
   - ⚠️ **주의**: Vercel 환경 변수에 `OPENAI_API_KEY`가 설정되어 있어야 합니다.
...
- ✅ OpenAI API 키는 Vercel의 Environment Variables에만 저장됩니다.
- ✅ 클라이언트 코드에는 API 키가 포함되지 않습니다.
- ✅ 모든 API 호출은 Vercel Functions를 통해 서버 측에서 처리됩니다.
- ⚠️ 프로덕션 환경에서는 CORS 설정을 적절히 조정하세요.
